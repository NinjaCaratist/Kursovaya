# BentleyOttmann реализация линии развертки
# (для нахождения всех пересечений в наборе отрезков)
...
__all__ = (
    ...
    # то же, что и выше, но включает сегменты с каждым пересечением
...
# ----------------------------------------------------------------------------
#Главное пересечение Полигонов

# Определяет, как изменить поведение алгоритмв.
#
# Следует ли игнорировать пересечения отрезков, 
# когда обе их конечные точки находятся в точке пересечения.
...
# Поддержка вертикальных сегментов,
# (метод Бентли-оттманна это не поддерживает).
#  используем термин "START_VERTICAL" для вертикального сегмента,
# чтобы отличить его от НАЧАЛА/КОНЦА/ПЕРЕСЕЧЕНИЯ
USE_VERTICAL = True
# ------------
...
# -----------------------------------------------------------------------------
# Реализация Переключаемого Номера
...
class Event:
    ...
    def __init__(self, type, point, segment, slope):
        ...
        # будет ли None for INTERSECTION
        self.slope = slope
    ...
    def y_intercept_x(self, x: Real):
        # вертикальные события только для сравнения (проверка above_all)
        if USE_VERTICAL:
            if self.is_vertical():
                return None
    ...
    def Compare(sweep_line, this, that):
        ...
        #ПРИМЕЧАНИЕ, ЗДЕСЬ ОЧЕНЬ ВАЖНО ИСПОЛЬЗОВАТЬ EPSILON!
    # в противном случае с ошибками точности с плавающей точкой мы получим неверные сравнения
    # может получиться очень странный и сложный для отладки вывод без этого.
...
class SweepLine:
    __slots__ = (
        #  Карта, содержащая все точки пересечения, сопоставленные с событиями
        # которые образуют эти пересечения.
        # {Point: set(Event, ...), ...}
        "intersections",
        "queue",

        # События (отсортированный набор упорядоченных событий, без значений)
      # примечание: события START & END считаются одинаковыми, поэтому проверка наличия события в дереве
      # вернет значение true, если будет найдена его противоположная сторона.
      # Это важно для работы алгоритма, и поэтому мы явно не удаляем события START.
      # Вместо этого END события никогда не добавляются в текущую развертку, и их удаление также удаляет начало.
        "_events_current_sweep",
        # Смысл текущего события
        "_current_event_point_x",
        # Флажок, указывающий, находимся ли мы немного раньше или позже линии.
        "_before",
    )
    ...
    # Проверяет, существует ли пересечение между двумя событиями'a' и'b'.
    def _check_intersection(self, a: Event, b: Event):
        # return, если одно из событий равно нулю, или
        #, если одно из них является событием INTERSECTION  .
        if (
        ...
        # Найти точку пересечения между'a' и 'b'.
        ...
        # Если пересечение образовано обоими концами сегмента, И
        # Значение USE_IGNORE_SEGMENT_ENDINGS равно true,
        # возврат из этого метода.
        ...
        # Если пересечение происходит справа от линии развертки ИЛИ
        # если пересечение находится на линии развертки и оно выше
        # текущей точки события, добавляет его в очередь как новое событие.
    ...
    def handle_event(self, event):
        ...
        if t == Event.Type.START:
            ...
            # примечание: events_current не отсортированы.
            reinsert_stack = []  # Stack
            for e in event_set:
                # Поскольку мы знаем, что событие еще не было удалено,
                # мы хотим вставить его позже.
                if self.remove(e):
                    reinsert_stack.append(e)
            self._before = False

            # Вставка всех событий, которые удалены.
            ...
            # В этом случае нам нужно только найти все сегменты в этом промежутке.
...
def isect_segments_impl(segments, *, include_segments=True, validate=True) -> list:
    # порядок точек left -> right
    if Real is float:
        segments = [
            # почти во всех случаях сравнения X достаточно,
            # сравнение Y и для вертикальных линий
    ...
    #Убедиться, что сегменты не имеют дубликатов или одиночных точек 
    if validate:
...
def isect_seg_seg_v2_point(v1, v2, v3, v4, bias=NUM_ZERO):
    #Только для  точки хеширования, когда вводится то же значение 
    if v1 > v2:
        v1, v2 = v2, v1
    if v3 > v4:
        v3, v4 = v4, v3
...
class _ABCTree(object):
    ...
    def pop_item(self):
        ...
        self.remove(key)
        return key, value
    popitem = pop_item  # для совместимости с dict()
    ...
    def succ_item(self, key, default=_sentinel):
        ...
        if node.right is not None:
            # найти самый маленький узел левого поддерева
            node = node.right
            while node.left is not None:
                node = node.left
            if succ_node is None:
                ...
                succ_node = node
        elif succ_node is None:  # заданный ключ самый большой в дереве
            if default is _sentinel:
                raise KeyError(str(key))
            return default
    ...
    def prev_item(self, key, default=_sentinel):
        ...
        # найден узел ключа 
        if node.left is not None:
            # найти самый большой узел левого поддерева 
            node = node.left
            while node.right is not None:
                node = node.right
            if prev_node is None:
                ...
                prev_node = node
        elif prev_node is None:  # заданный ключ самый маленький в дереве 
            if default is _sentinel:
                raise KeyError(str(key))
            return default
    ...
    def set_default(self, key, default=None):
        ...
        if value is _sentinel:
            ...
            return default
        return value
    setdefault = set_default  # для совместимости с dict()
...
class RBTree(_ABCTree):
    ...
    def insert(self, key, value):
        if self._root is None:  # Если дерево пустое
            self._root = self._new_node(key, value)
            self._root.red = False  # сделать корень черный
            return

        head = Node()  # False корень
        grand_parent = None
        ...
        # Поиск по дереву
        while True:
            if node is None:  # Вставить новый узел внизу 
                node = self._new_node(key, value)
                parent[direction] = node
            elif RBTree.is_red(node.left) and RBTree.is_red(node.right):  # Сменп цвета
                node.red = True
                node.left.red = False
            ...
            #остановка, если нашло
            if self._cmp(self._cmp_data, key, node.key) == 0:
                node.value = value  # установить новое значение для ключа 
                break
        ...
        self._root = head.right  # обновитть корень
        self._root.red = False  # Сделать корень черным
    ...
    def remove(self, key):
        ...
        while node[direction] is not None:
            ...
            # сохранит найденный узел
            if self._cmp(self._cmp_data, key, node.key) == 0:
                found = node

            # опустить красный узел вниз
        ...
        # Обновите root и сделать его черным
